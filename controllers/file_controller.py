...
def init_upload_logic(tender_file, bid_files):
    ...
    try:
        ...
        single_file_max_size = runtime_config.get('single_file_max_size', 100 * 1024 * 1024)  # 默认100MB
        ...
        if tender_file and tender_file.filename != '':
            if tender_file.content_length > single_file_max_size:
                ...
                raise RequestEntityTooLarge(f'招标文件大小超出限制，请上传小于{max_mb}MB的文件')
        ...
        for bid_file in bid_files:
            if bid_file and bid_file.filename != '':
                if bid_file.content_length > single_file_max_size:
                    max_mb = single_file_max_size // (1024 * 1024)
                    raise RequestEntityTooLarge(f'投标文件大小超出限制，请上传小于{max_mb}MB的文件')
        ...
        for bid_file in bid_files:
            ...
            bid_files_info.append({
                ...
                'bidder_name': bid_document.bidder_name if bid_document.bidder_name else '',
            })
        ...
        # 从运行时配置获取文件大小限制
        runtime_config = load_config()
        single_file_max_size = runtime_config.get('single_file_max_size', 100 * 1024 * 1024)
        max_mb = single_file_max_size // (1024 * 1024)
...
def list_project_bidders_logic(project_id):
    ...
    try:
        ...
        # 查询项目下的所有投标文件
        bid_documents = db.query(BidDocument).filter(
        ...
        for doc in bid_documents:
            bidders.append({
                'id': int(doc.id),
                'original_filename': doc.original_filename if doc.original_filename else '',
                'bidder_name': doc.bidder_name if doc.bidder_name else '',
                'processing_status': doc.processing_status if doc.processing_status else '',

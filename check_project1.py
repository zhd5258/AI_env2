...
#
# 作者           : KingFreeDom
# 创建时间         : 2025-09-22 22:20:04
# 最近一次编辑者      : KingFreeDom
# 最近一次编辑时间     : 2025-09-22 22:20:06
# 文件相对于项目的路径   : \AI_env2\check_project1.py
#
...
def check_project1():
    ...
    try:
        ...
        # 检查项目1
        project = session.query(TenderProject).filter(TenderProject.id == 1).first()
        if not project:
            print('错误：项目1不存在')
            return
        ...
        # 检查评分规则
        scoring_rules = (
            session.query(ScoringRule).filter(ScoringRule.project_id == 1).all()
        )
        ...
        for doc in bid_docs:
            ...
            print(f'    部分分析结果: {doc.partial_analysis_results}')

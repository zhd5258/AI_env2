    ...
    try:
        ...
        if success:
            ...
            try:
                ...
                # 分析保存的规则
                parent_rules = [r for r in rules if r.Parent_Item_Name is not None and r.Child_Item_Name is None]
                child_rules = [r for r in rules if r.Child_Item_Name is not None]
                incomplete_rules = [r for r in rules if r.Parent_Item_Name is not None and r.Child_Item_Name is None]
                
                print(f"  - 父项规则: {len(parent_rules)} 条")
                print(f"  - 子项规则: {len(child_rules)} 条")
                print(f"  - 不完整规则: {len(incomplete_rules)} 条")
...
def test_rule_inheritance_and_cleanup():
    """
    测试规则继承和清理逻辑
    """
    print("测试规则继承和清理逻辑...")
...
if __name__ == "__main__":
    test_rule_inheritance_and_cleanup()
    print("\n" + "="*50 + "\n")

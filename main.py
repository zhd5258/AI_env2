...
#
# 作者           : KingFreeDom
# 创建时间         : 2025-09-25 21:07:18
# 最近一次编辑者      : KingFreeDom
# 最近一次编辑时间     : 2025-09-27 10:17:08
# 文件相对于项目的路径   : \AI_env2\main.py
#
...
# 抑制PaddlePaddle警告
try:
    from modules.paddle_warning_suppressor import setup_paddle_environment
...
def initialize_project_analysis(project_id: int):
    ...
    try:
        ...
        return analysis_manager.initialize_project_analysis(project_id)
...
@app.delete('/api/projects/{project_id}/bid-documents/batch')
async def delete_bid_documents_batch(
    ...
    try:
        ...
        if not bid_ids:
            return JSONResponse(
                status_code=400, content={'error': '请提供要删除的投标文档ID列表'}
            )
        ...
        for bid_id in bid_ids:
            ...
            bid_doc = (
                ...
                .filter(BidDocument.id == bid_id, BidDocument.project_id == project_id)
                .first()
            )
            ...
            if bid_doc:
                ...
                db.query(AnalysisResult).filter(
                    AnalysisResult.bid_document_id == bid_id
                ).delete()

                # 删除文件系统中的文件
                file_path_str = str(bid_doc.file_path) if bid_doc.file_path else None
                if file_path_str and os.path.exists(file_path_str):
                    try:
                        os.remove(file_path_str)
                        logging.info(f'已删除文件: {file_path_str}')
                    except Exception as e:
                        logging.warning(f'删除文件失败 {file_path_str}: {e}')
        ...
        logging.info(
            f'批量删除投标文档完成：项目ID {project_id}，删除数量 {deleted_count}'
        )
        ...
        return JSONResponse(
            ...
            content={
                'success': True,
                'message': f'成功删除 {deleted_count} 个投标文档',
                'deleted_count': deleted_count,
                'deleted_files': deleted_files,
            },
        )
    except Exception as e:
        logging.error(f'批量删除投标文档失败: {e}')
...
def extract_pdf_text_background(file_path: str, bidder_name: str):
    ...
    try:
        ...
        future = local_executor.submit(
            processor.extract_text_per_page
        )  # 自动选择OCR引擎
        try:
...
def analysis_task(
    ...
    try:
        ...
        bid_document.progress_current_rule = '开始处理'  # type: ignore[assignment]
        db.commit()
        # 检查是否启用GPU（从环境变量中读取）
        use_gpu = os.getenv('USE_GPU', 'false').lower() == 'true'
        ...
        analyzer = IntelligentBidAnalyzer(
            tender_file_path,
            bid_document.file_path,  # type: ignore[reportArgumentType]
            db_session=db,
            bid_document_id=bid_document_id,
            project_id=project_id,
        ...
        if analysis_result['status'] == 'success':
            ...
            bid_document.progress_current_rule = '分析完成'  # type: ignore[assignment]
            # 记录提取到的投标人名称和投标总价
            if 'details' in analysis_result:
                ...
                if isinstance(details, dict):
                    if (
                        '投标人名称' in details
                        and details['投标人名称'] != '待分析确认'
                    ):
                        bid_document.bidder_name = details['投标人名称']  # type: ignore[assignment]
                    # 检查是否是AI分析成功的结果，如果是则直接使用，不再重复处理
                    ai_success = details.get('_ai_analysis_success', False)
                    if '投标总价' in details and details['投标总价'] != '未提取':
                        if ai_success:
                            logging.info(
                                f'AI已成功提取投标总价: {details["投标总价"]}，跳过重复处理'
                            )
                        else:
                            # 只有在AI分析失败时才使用价格提取管理器
                            try:
                                # 尝试保存投标总价到数据库
                                ...
                                price_value = price_manager.extract_and_select_price(
                                    pages
                                )
            ...
            bid_document.error_message = analysis_result['message']  # type: ignore[assignment]
            bid_document.progress_current_rule = '分析失败'  # type: ignore[assignment]
...
def run_analysis_and_calculate_prices(project_id: int, bid_files_info: list):
    ...
    try:
        analysis_manager = AnalysisManager(db_session=db)
        analysis_manager.run_analysis_and_calculate_prices(project_id, bid_files_info)
    except Exception as e:
        logging.error(f'为项目 {project_id} 执行分析时出错: {e}')
    ...
    try:
        ...
        for bidder_update in bidders_updates:
            ...
            if doc and confirmed_name:
                # 更新名称和状态
                doc.bidder_name = confirmed_name  # type: ignore[assignment]
                doc.processing_status = 'pending'  # type: ignore[assignment]
                doc.progress_current_rule = '准备中...'  # type: ignore[assignment]
                # 确保对应的分析结果记录存在
                analysis_result = (
                    db.query(AnalysisResult)
        ...
        project.status = 'processing'  # type: ignore[assignment]
        db.commit()
        try:
            # Simulate a long-running task
            time.sleep(10)
            project.status = 'completed'  # type: ignore[assignment]  # type: ignore[assignment]
            db.commit()
        except Exception:
        ...
        if bid_files_info:
            ...
            background_tasks.add_task(
                analysis_manager.run_analysis_and_calculate_prices,
                project_id,
                bid_files_info,
            )
        else:
...
async def get_analysis_status(project_id: int, db: Session = Depends(get_db)):
    ...
    for doc in bid_documents:
        ...
        if doc.processing_status not in ['completed', 'error']:
            all_completed = False
        if doc.processing_status == 'error':  # type: ignore[reportGeneralTypeIssues]
            has_errors = True
...
async def get_scoring_rules(project_id: int, db: Session = Depends(get_db)):
    ...
    if project.scoring_rules_summary:  # type: ignore[reportGeneralTypeIssues]
        # 直接返回缓存的评分规则摘要（若已存在）
    ...
    for rule in scoring_rules:
        parent_name = getattr(rule, 'Parent_Item_Name', None)
        child_name = getattr(rule, 'Child_Item_Name', None)
        child_max = getattr(rule, 'Child_max_score', None)
        parent_max = getattr(rule, 'Parent_max_score', None)
        description = getattr(rule, 'description', None)
        is_veto = bool(getattr(rule, 'is_veto', False))
        criteria_name_value = (
            child_name
            if (child_name is not None and str(child_name) != '')
            else parent_name
        )
        max_score_value = child_max if (child_max is not None) else parent_max
        response_data.append(
            {
                'id': getattr(rule, 'id', None),
                'category': parent_name,
                'criteria_name': criteria_name_value,
...
async def get_all_projects(db: Session = Depends(get_db)):
    ...
    for project in projects:
        if getattr(project, 'created_at', None) is not None:  # 避免对ORM列做布尔判断
            response_data.append(
                {
                    'id': project.id,
...
async def get_failed_pages_info(
    ...
    if not bid_document.failed_pages_info:  # type: ignore[reportGeneralTypeIssues]
        return JSONResponse(
            status_code=404,
            content={
    ...
    try:
        failed_pages_data = json.loads(bid_document.failed_pages_info)  # type: ignore[reportArgumentType]
    ...
    try:
        ...
        return JSONResponse(
            content={
                ...
                'updated_count': len(price_scores),  # type: ignore[reportArgumentType]
            }
        )
    except Exception as e:
    ...
    try:
        ...
        for bid_file in bid_files:
            if bid_file.filename:
                ...
                # 更新投标文档记录的投标人名称
                bid_document.bidder_name = default_bidder_name  # type: ignore[assignment]
        ...
        def _safe_init_wrapper(pid: int):
            try:
                initialize_project_analysis(pid)
            except Exception as e:
                logging.error(f'初始化项目分析失败: {e}', exc_info=True)

        # 启动后台线程进行评分规则提取
        pid_val = cast(int, project.id)
        thread = threading.Thread(target=_safe_init_wrapper, args=(pid_val,))
        thread.start()
        return JSONResponse(
            content={
                'project_id': project.id,
...
async def bulk_update_scores(
    ...
    for update in score_updates:
        ...
        if result:
            result.total_score = update.total_score  # type: ignore[reportAttributeAccessIssue]
            updated_count += 1
    if updated_count > 0:
        db.commit()
...
async def extract_scoring_rules_api(
    ...
    try:
        ...
        if not project:
            return JSONResponse(status_code=404, content={'error': '项目不存在'})
        # 检查项目是否有招标文件
        if not project.tender_file_path or not Path(project.tender_file_path).exists():  # type: ignore[reportArgumentType]
            return JSONResponse(status_code=404, content={'error': '招标文件不存在'})
        # 使用统一的分析管理器提取评分规则
        analysis_manager = AnalysisManager(db_session=db)
        extract_result = analysis_manager.initialize_project_analysis(project_id)
        if extract_result:
            # 获取提取的评分规则
            from modules.scoring_rules_manager import ScoringRulesManager

            rules_manager = ScoringRulesManager(db_session=db)
            scoring_rules = rules_manager.get_scoring_rules(project_id)
            # 转换为字典格式
            rules_data = []
            for rule in scoring_rules:
                rules_data.append(
...
async def export_project_results_excel(project_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 创建Excel文件
        output = BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:  # type: ignore[reportArgumentType]
            # 创建汇总表
            if 'header_rows' in summary_data and len(summary_data['header_rows']) >= 2:
                ...
                if (
                    ...
                    columns.append('价格分')
                    columns.append('总分')
                    df = pd.DataFrame(rows_data, columns=[str(c) for c in columns])  # type: ignore[arg-type]
                else:
                    # 默认列名
                    columns = ['排名', '投标人']
                    # 添加子项列名
            ...
            for result in results:
                # 创建详细评分数据
                detailed_data = []
                if result.detailed_scores:  # type: ignore[reportGeneralTypeIssues]
                    detailed_scores = (
                    ...
                    if isinstance(detailed_scores, list):
                        for score_item in detailed_scores:
                            ...
                            if 'Child_Item_Name' in score_item:
                                detailed_data.append(
                                    {
                                        '评分项': score_item.get('Child_Item_Name', ''),
                            ...
                            # 旧格式：包含criteria_name、max_score、score、reason等字段
                                ...
                                detailed_data.append(
                                    {
                                        '评分项': score_item.get('criteria_name', ''),
                                        '满分': score_item.get('max_score', ''),
                                        '得分': score_item.get('score', ''),
                                        '评分说明': score_item.get('reason', ''),
                                    }
                        ...
                        # 旧格式：字典形式
                        for criteria_name, score_info in detailed_scores.items():
                ...
                if detailed_data:
                    ...
                    sheet_name = (
                        ...
                        if getattr(result, 'bidder_name', None) is not None
...
async def export_project_results_word(project_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        doc.add_paragraph(
            f'创建时间: {project.created_at.strftime("%Y-%m-%d %H:%M:%S") if project.created_at else "N/A"}'  # type: ignore[reportGeneralTypeIssues]
        )
        # 添加汇总表
        ...
        for result in results:
            ...
            # 添加基本信息
            doc.add_paragraph(
                f'投标总价: {result.extracted_price if result.extracted_price else "N/A"}'  # type: ignore[reportGeneralTypeIssues]
            )
            doc.add_paragraph(f'AI模型: {result.ai_model or "N/A"}')
            # 添加详细评分表
            if getattr(result, 'detailed_scores', None) is not None:
                try:
                    scores = (
                        json.loads(result.detailed_scores)
                        if isinstance(result.detailed_scores, str)
                    ...
                    if scores and isinstance(scores, (list, dict)) and len(scores) > 0:  # type: ignore[arg-type]
                        # 创建详细评分表格
                        table = doc.add_table(rows=1, cols=4)
                        table.style = 'Table Grid'
                        hdr_cells = table.rows[0].cells

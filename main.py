...
def analysis_task(project_id: int, bid_document_id: int):
    ...
    try:
        ...
        if not tender_file_path or not os.path.exists(str(tender_file_path)):
            logging.error('招标文件不存在 %s', tender_file_path)
            bid_document.processing_status = 'error'  # type: ignore
            bid_document.error_message = '招标文件不存在'  # type: ignore
            bid_document.progress_current_rule = '分析失败'  # type: ignore
            db.commit()
            return

        bid_document.processing_status = 'processing'  # type: ignore
        bid_document.progress_completed_rules = 0  # type: ignore
        bid_document.progress_total_rules = 0  # type: ignore
        bid_document.progress_current_rule = '初始化分析...'  # type: ignore
        db.commit()

        analyzer = IntelligentBidAnalyzer(
            str(tender_file_path) if tender_file_path else '',
            str(bid_document.file_path) if bid_document.file_path else '',
            db_session=db,
        ...
        def run_analysis():
            ...
            try:
                result_data = analyzer.analyze()  # type: ignore
            except Exception as e:
                analysis_error = e
        ...
        try:
            ...
            if analysis_thread.is_alive():
                ...
                bid_document.processing_status = 'error'  # type: ignore
                bid_document.error_message = '分析超时，请重试'  # type: ignore
                bid_document.progress_current_rule = '分析超时'  # type: ignore
                db.commit()
                return
            elif analysis_error:
                logging.error(
                ...
                bid_document.processing_status = 'error'  # type: ignore
                bid_document.error_message = f'分析异常: {str(analysis_error)}'  # type: ignore
                bid_document.progress_current_rule = '分析异常'  # type: ignore
                db.commit()
                return
            else:
            ...
            bid_document.processing_status = 'error'  # type: ignore
            bid_document.error_message = f'分析异常: {str(e)}'  # type: ignore
            bid_document.progress_current_rule = '分析异常'  # type: ignore
            db.commit()
            return
        ...
        if result_data is None:
            ...
            bid_document.processing_status = 'error'  # type: ignore
            bid_document.error_message = '分析结果为空'  # type: ignore
            bid_document.progress_current_rule = '分析失败'  # type: ignore
            db.commit()
            return
        ...
        if not isinstance(result_data, dict):
            ...
            bid_document.processing_status = 'error'  # type: ignore
            bid_document.error_message = '分析结果格式错误'  # type: ignore
            bid_document.progress_current_rule = '分析失败'  # type: ignore
            db.commit()
            return
        ...
        if 'error' in result_data:
            ...
            bid_document.processing_status = 'error'  # type: ignore
            bid_document.error_message = result_data['error']  # type: ignore
            bid_document.progress_current_rule = '分析出错'  # type: ignore
            db.commit()
            return
        ...
        bid_document.processing_status = 'completed'  # type: ignore
        db.commit()
        ...
        if db and bid_document:
            bid_document.processing_status = 'error'  # type: ignore
            bid_document.error_message = f'Critical error: {str(e)}'  # type: ignore
            db.commit()
    finally:
        if db:
            db.close()
...
def run_analysis_and_calculate_prices(project_id: int, bid_files_info: list):
    ...
    try:
        ...
        if (
            ...
            and os.path.exists(str(project.tender_file_path))
        ):
            # 检查是否已存在评分规则
            existing_rules = (
            ...
            if existing_rules == 0:
                ...
                scoring_rules = extractor.extract(str(project.tender_file_path))
                ...
                if scoring_rules:
                    ...
                    def save_rule_recursive(rule_data, project_id, parent_id=None):
                        ...
                        # 如果是价格规则，设置价格公式字段
                        if bool(db_rule.is_price_criteria):
                            db_rule.price_formula = None  # type: ignore
                            db_rule.Child_Item_Name = None  # type: ignore
                            db_rule.Child_max_score = None  # type: ignore
                        else:
                            ...
                            if 'children' in rule_data and rule_data['children']:
                                ...
                                # 叶子节点规则（没有子项）
                                db_rule.Child_Item_Name = rule_data.get('criteria_name')  # type: ignore
                                db_rule.Child_max_score = rule_data.get('max_score')  # type: ignore
    ...
    try:
        ...
        if price_scores:
            logging.info(
                f'项目 {project_id} 价格分计算完成，更新了{len(list(price_scores))} 个投标方。'
            )
        else:
        ...
        if project:
            ...
            project.status = 'completed_with_errors' if has_errors else 'completed'  # type: ignore
            db.commit()
            logging.info(f'项目 {project_id} 的状态已更新为{project.status}。')  # type: ignore
...
async def analyze_immediately(
    ...
    tender_file_path = save_upload_file(
        tender_file,
        os.path.join(UPLOADS_DIR, f'{int(project.id)}_tender_{tender_file.filename}'),  # type: ignore
    )
    project.tender_file_path = str(tender_file_path)  # type: ignore
    db.commit()
    ...
    for bid_file in bid_files:
        bid_file_path = save_upload_file(
            bid_file,
            os.path.join(UPLOADS_DIR, f'{int(project.id)}_bid_{bid_file.filename}'),  # type: ignore
        )
        ...
        if not bidder_name:
            bidder_name = (
                os.path.splitext(bid_file.filename)[0]  # type: ignore
...
async def get_analysis_status(project_id: int, db: Session = Depends(get_db)):
    ...
    for doc in bid_documents:
        ...
        if doc.partial_analysis_results:
            try:
                partial_results = json.loads(str(doc.partial_analysis_results))
            except json.JSONDecodeError as e:
                logging.error('解析部分分析结果失败: %s', e)

        status_data.append(
            {
                'bidder_name': str(doc.bidder_name) if doc.bidder_name else '',
                'status': str(doc.processing_status) if doc.processing_status else '',
                'error_message': str(doc.error_message) if doc.error_message else '',
                'progress_completed': int(doc.progress_completed_rules)
                if doc.progress_completed_rules
                else 0,
                'progress_total': int(doc.progress_total_rules)
                if doc.progress_total_rules
                else 0,
                'current_rule': str(doc.progress_current_rule)
                if doc.progress_current_rule
                else '',
                'detailed_progress_info': str(doc.detailed_progress_info)
                if doc.detailed_progress_info
                else '',
        ...
        if (
            str(doc.processing_status) != 'completed'
            and str(doc.processing_status) != 'error'
        ):
            all_completed = False
        if str(doc.processing_status) == 'error':
            has_errors = True

    if all_completed and str(project.status) != 'completed':
        project.status = 'completed_with_errors' if has_errors else 'completed'  # type: ignore
        db.commit()

    return JSONResponse(
        content={
            'project_status': str(project.status) if project.status else '',
            'bids': status_data,
        }
    )
...
async def get_analysis_results(project_id: int, db: Session = Depends(get_db)):
    ...
    for res in results:
        ...
        response_data.append(
            {
                'id': res.id,
                'bidder_name': str(res.bidder_name) if res.bidder_name else '',
                'total_score': float(res.total_score)
                if res.total_score is not None
                else 0.0,
                'price_score': float(price_score) if price_score is not None else None,
                'extracted_price': float(res.extracted_price)
                if res.extracted_price is not None
                else None,
                'detailed_scores': json.loads(res.detailed_scores)
                if isinstance(res.detailed_scores, str)
                else res.detailed_scores,
                'ai_model': str(res.ai_model) if res.ai_model else '',
            }
        )
...
async def get_scoring_rules(project_id: int, db: Session = Depends(get_db)):
    ...
    if project.scoring_rules_summary:
        if isinstance(project.scoring_rules_summary, list):
            ...
            logging.warning(
                f'Project {project_id} 的scoring_rules_summary 格式不正确，将从 ScoringRule 表中回退。'
            )
    ...
    for rule in scoring_rules:
        response_data.append(
            {
                'id': rule.id,
                'category': str(rule.Parent_Item_Name) if rule.Parent_Item_Name else '',
                'criteria_name': str(rule.Child_Item_Name)
                if rule.Child_Item_Name
                else str(rule.Parent_Item_Name)
                if rule.Parent_Item_Name
                else '',
                'max_score': float(rule.Child_max_score)
                if rule.Child_max_score is not None
                else float(rule.Parent_max_score)
                if rule.Parent_max_score is not None
                else 0.0,
                'weight': 1.0,
                'description': str(rule.description) if rule.description else '',
                'is_veto': bool(rule.is_veto) if rule.is_veto is not None else False,
            }
        )
        ...
        response_data.append(
            {
                ...
                'name': str(project.name) if project.name else '',
                'description': str(project.description) if project.description else '',
                'created_at': project.created_at.isoformat()
                if project.created_at  # type: ignore
                else None,
...
async def get_failed_pages_info(
    ...
    try:
        failed_pages_data = json.loads(str(bid_document.failed_pages_info))
...
async def recalculate_price_scores(project_id: int, db: Session = Depends(get_db)):
    try:
        ...
        incomplete_bids = [
            doc
            for doc in bid_documents
            if str(doc.processing_status) != 'completed'
            and str(doc.processing_status) != 'error'
        ]
        if incomplete_bids:
            return JSONResponse(
        ...
        return JSONResponse(
            content={
                ...
                'updated_count': len(list(price_scores)),
            }
        )
...
async def bulk_update_scores(
    ...
    for update in score_updates:
        ...
        if result:
            result.total_score = float(update.total_score)  # type: ignore
            updated_count += 1
...
async def extract_scoring_rules_api(
    ...
    try:
        ...
        # 检查项目是否有招标文件
        if not project.tender_file_path or not os.path.exists(

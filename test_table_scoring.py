...
#
# 作者           : KingFreeDom
# 创建时间         : 2025-09-10 21:45:20
# 最近一次编辑者      : KingFreeDom
# 最近一次编辑时间     : 2025-09-10 21:45:23
# 文件相对于项目的路径   : \AI_env2\test_table_scoring.py
#
...
def test_table_scoring_extraction():
    """测试表格评分规则提取功能"""
    print('开始测试表格评分规则提取功能...')
    ...
    try:
        ...
        if rules:
            ...
            # 保存结果到JSON文件
            output_file = 'table_scoring_rules_test.json'
...
def test_full_document_extraction():
    ...
    try:
        ...
        if rules:
            ...
            # 保存结果到JSON文件
            output_file = 'full_document_scoring_rules_test.json'
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(rules, f, ensure_ascii=False, indent=2)
            print(f'\n评分规则已保存到: {output_file}')
...
if __name__ == '__main__':
    ...
    # 测试表格区域的评分规则提取
    table_rules = test_table_scoring_extraction()

    # 测试完整文档的评分规则提取
    full_rules = test_full_document_extraction()

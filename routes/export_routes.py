    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            project_info = {
                ...
                '处理状态': [project.processing_status],
                '投标文件数量': [len(results)],
...
def export_project_results_word(project_id):
    ...
    try:
        ...
        header_cells[1].text = '投标人'
        header_cells[2].text = '总分'
        header_cells[3].text = '投标价格'
        ...
        for rank, result in enumerate(sorted_results, 1):
            ...
            row_cells[2].text = (
                f'{result.total_score:.2f}' if result.total_score else '0.00'
            )
            row_cells[3].text = (
                f'{result.extracted_price:,.2f}' if result.extracted_price else '未提取'
            )

        # 详细评分
        doc.add_heading('3. 详细评分情况', level=1)

        for rank, result in enumerate(sorted_results, 1):
            doc.add_heading(f'3.{rank} {result.bidder_name}', level=2)

            # 基本信息
            basic_info = doc.add_paragraph()
            basic_info.add_run(
                f'总分: {result.total_score:.2f if result.total_score else 0.00}'
            ).bold = True
            basic_info.add_run(
                f"\n投标价格: {result.extracted_price:,.2f if result.extracted_price else '未提取'}"
            )
            ...
            try:
                if result.detailed_scores:
                    ...
                    if isinstance(detailed_scores, list) and detailed_scores:
                        doc.add_paragraph('详细评分:', style='List Bullet')

                        for score_item in detailed_scores:
                            if isinstance(score_item, dict):
                                for score_name, score_value in score_item.items():
                                    doc.add_paragraph(
                                        f'{score_name}: {score_value:.2f if isinstance(score_value, (int, float)) else score_value}',
                                        style='List Bullet 2',
                                    )
            except (json.JSONDecodeError, TypeError):
                doc.add_paragraph('详细评分解析失败', style='List Bullet')

        # 评分规则
        if scoring_rules:
            doc.add_heading('4. 评分规则', level=1)

            rules_table = doc.add_table(rows=1, cols=5)
            rules_table.style = 'Table Grid'

            # 表头
            rule_header_cells = rules_table.rows[0].cells
            rule_header_cells[0].text = '父级评分项'
            rule_header_cells[1].text = '子级评分项'
            rule_header_cells[2].text = '父级最高分'
            rule_header_cells[3].text = '子级最高分'
            rule_header_cells[4].text = '评分说明'
            ...
            for rule in scoring_rules:
                ...
                row_cells[2].text = (
                    str(rule.Parent_max_score) if rule.Parent_max_score else ''
                )
                row_cells[3].text = (
                    str(rule.Child_max_score) if rule.Child_max_score else ''
                )

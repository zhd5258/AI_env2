...
def migrate_database():
    ...
    if 'analysis_start_time' not in tender_project_table.c:
        ...
        with engine.connect() as conn:
            ...
            conn.execute(
                text(
                    'ALTER TABLE tender_project ADD COLUMN analysis_start_time DATETIME'
                )
            )
            conn.commit()
    ...
    if 'bid_document' in metadata.tables:
        ...
        if 'original_filename' not in bid_document_table.c:
            ...
            with engine.connect() as conn:
                ...
                conn.execute(
                    text(
                        'ALTER TABLE bid_document ADD COLUMN original_filename VARCHAR'
                    )
                )
                conn.commit()
        ...
        if 'ocr_retry_count' not in bid_document_table.c:
            ...
            with engine.connect() as conn:
                ...
                conn.execute(
                    text(
                        'ALTER TABLE bid_document ADD COLUMN ocr_retry_count INTEGER DEFAULT 0'
                    )
                )
                conn.commit()
    ...
    # 检查scoring_rule表是否存在
    if 'scoring_rule' in metadata.tables:
        # 获取现有的scoring_rule表
        scoring_rule_table = metadata.tables['scoring_rule']

        # 检查是否需要添加needs_comprehensive_analysis列
        if 'needs_comprehensive_analysis' not in scoring_rule_table.c:
            print('添加needs_comprehensive_analysis列...')
            with engine.connect() as conn:
                # 使用ALTER TABLE语句添加列
                conn.execute(
                    text(
                        'ALTER TABLE scoring_rule ADD COLUMN needs_comprehensive_analysis BOOLEAN DEFAULT 0'
                    )
                )
                conn.commit()
            print('needs_comprehensive_analysis列添加完成')
        else:
            print('needs_comprehensive_analysis列已存在')

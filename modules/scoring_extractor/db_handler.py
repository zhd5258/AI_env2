...
class DBHandlerMixin:
    ...
    def save_scoring_rules_to_db(
        self, project_id: int, rules: List[Dict[str, Any]]
    ) -> bool:
        """
        ...
        try:
            ...
            with self._get_db_session() as db:
                ...
                save_result = rules_manager.save_scoring_rules(project_id, rules)
                ...
                if save_result:
                    ...
                    self.logger.info(
                        f'成功将评分规则保存到数据库并完成清理，项目ID: {project_id}'
                    )
                    return True
                else:
                    self.logger.error(f'保存评分规则到数据库失败，项目ID: {project_id}')
                    return False
        except Exception as e:
    ...
    def save_scoring_rules_from_table_data(
        self, project_id: int, structured_tables: List[Dict]
    ) -> bool:
        """
        ...
        try:
            ...
            scoring_rules = parser.parse_scoring_rules_from_table_data(
                structured_tables
            )
    ...
    def _parse_scoring_rules_from_table_data(
    ...
    def _parse_scoring_rules_from_table_data_impl(
        self, structured_tables: List[Dict]
    ) -> List[Dict[str, Any]]:
        """

...
class MDPriceExtractor:
        ...
        if target_sections:
            for i, section in enumerate(target_sections):
                ...
                if price is not None and price > 1000:  # 过滤过小的价格
                    self.logger.info(f'从第 {i+1} 个投标一览表区域提取到价格: {price}')
                    return price
        ...
        for i, line in enumerate(lines):
            for keyword in self.bid_summary_keywords:
                if keyword in line:
                    ...
                    self.logger.info(f'提取了从第 {start_idx+1} 行到第 {end_idx} 行的段落')
                    break  # 找到一个关键词就足够了
        ...
        if small_number_price is None and large_number_price is None:
            for keyword in self.price_field_keywords:
                ...
                if match:
                    ...
                    self.logger.info(f'从投标一览表的投标总价列提取到阿拉伯数字价格: {small_number_price} (原始字符串: {price_str})')
                    if small_number_price is not None:
                        ...
                        if not any(exclude in line_text for exclude in self.exclude_keywords):
                            # 基础置信度
            ...
            if chinese_match:
                ...
                large_number_price = self._chinese_to_number(chinese_num)
                self.logger.info(f'从投标一览表的投标总价列提取到汉字大写价格: {large_number_price} (原始字符串: {chinese_num})')
        ...
        if small_number_price is not None and large_number_price is not None:
            ...
            if abs(small_number_price - large_number_price) < 1000:  # 允许一定误差
                ...
                if small_confidence > large_confidence:
                    self.logger.debug(f'小写金额置信度更高，返回小写金额: {small_number_price}')
                    return small_number_price
                else:
                    self.logger.debug(f'大写金额置信度更高，返回大写金额: {large_number_price}')
                    return large_number_price
        
        # 4. 如果只有一种格式存在，则根据其置信度决定是否返回
        if small_number_price is not None:
            self.logger.debug(f'只找到小写金额: {small_number_price}(置信度{small_confidence})')
            if small_confidence >= 85.0:  # 高置信度才返回
                self.logger.info(f'小写金额置信度足够高，返回: {small_number_price}')
                return small_number_price
            else:
                self.logger.info(f'小写金额置信度不足，不返回')
        
        if large_number_price is not None:
            self.logger.debug(f'只找到大写金额: {large_number_price}(置信度{large_confidence})')
            if large_confidence >= 85.0:  # 高置信度才返回
                self.logger.info(f'大写金额置信度足够高，返回: {large_number_price}')
                return large_number_price
            else:
                self.logger.info(f'大写金额置信度不足，不返回')
            ...
            if header_index + 1 < len(table_lines):
                ...
                for cell in cells:
                    ...
                    if small_match:
                        ...
                        small_number_price = self._str_to_float(price_str)
                        self.logger.info(f'从表格中提取到阿拉伯数字价格: {small_number_price} (原始字符串: {price_str})')
                        if small_number_price is not None:
                            # 计算置信度
                            small_confidence = 70.0
                            # 如果有千位分隔符，则增加置信度
                    ...
                    if large_match:
                        ...
                        large_number_price = self._chinese_to_number(chinese_num)
                        self.logger.info(f'从表格中提取到汉字大写价格: {large_number_price} (原始字符串: {chinese_num})')
                        if large_number_price is not None:
                            # 计算置信度
                            large_confidence = 60.0
                            # 根据中文数字的完整性评估置信度
                ...
                if small_number_price is not None and large_number_price is not None:
                    self.logger.info(f'找到两个价格: 阿拉伯数字={small_number_price}(置信度{small_confidence}), 汉字大写={large_number_price}(置信度{large_confidence})')
                    # 如果两者相差很小，则认为是正确的，直接返回小写金额
                    if abs(small_number_price - large_number_price) < 1:
                        ...
                        self.logger.debug('两个价格差异较大，比较置信度')
                        if small_confidence > large_confidence:
                            self.logger.info(f'阿拉伯数字置信度更高，返回阿拉伯数字价格: {small_number_price}')
                            return small_number_price
                        else:
                            self.logger.info(f'汉字大写置信度更高，返回汉字大写价格: {large_number_price}')
                            return large_number_price
                
                # 如果只有一种格式存在，则根据其置信度决定是否返回
                if small_number_price is not None:
                    self.logger.info(f'只找到阿拉伯数字价格: {small_number_price}(置信度{small_confidence})')
                    if small_confidence >= 85.0:  # 高置信度才返回
                        self.logger.info(f'阿拉伯数字价格置信度足够高，返回: {small_number_price}')
                        return small_number_price
                    else:
                        self.logger.info(f'阿拉伯数字价格置信度不足，不返回')
                
                if large_number_price is not None:
                    self.logger.info(f'只找到汉字大写价格: {large_number_price}(置信度{large_confidence})')
                    if large_confidence >= 85.0:  # 高置信度才返回
                        self.logger.info(f'汉字大写价格置信度足够高，返回: {large_number_price}')
                        return large_number_price
                    else:
                        self.logger.info(f'汉字大写价格置信度不足，不返回')
                ...
                for pattern in [
                    ...
                    if match:
                        price_str = match.group(1)
                        price = self._str_to_float(price_str)
                        self.logger.info(f'从表格中提取到价格: {price} (原始字符串: {price_str})')
                        if price is not None:
                            return price
    ...
    def _parse_markdown_table(self, section: str) -> list[dict[str, str]]:
        ...
        if len(table_lines) >= 2:
            ...
            headers = [h.strip() for h in header_line.split('|')[1:-1]]  # 去掉首尾的空列
            ...
            for key, value in row.items():
                ...
                if any(keyword in key for keyword in self.price_field_keywords):
                    ...
                    if small_match:
                        ...
                        small_number_price = self._str_to_float(price_str)
                        if small_number_price is not None:
                            self.logger.debug(f'从结构化表格中提取到小写价格: {small_number_price} (原始字符串: {price_str})')
                    ...
                    if large_number_price is not None and small_number_price is None:
                        ...
                        if small_match:
                            price_str = small_match.group(1)
                            small_number_price = self._str_to_float(price_str)
                            if small_number_price is not None and small_number_price > 1000:
                                self.logger.debug(f'从结构化表格中提取到小写价格: {small_number_price} (原始字符串: {price_str})')
    ...
    def _chinese_to_number(self, chinese_num: str) -> float | None:
        ...
        # 预处理：替换全角字符和可能的变体
        chinese_num = chinese_num.replace('壹', '一').replace('贰', '二')\
                                  .replace('叁', '三').replace('肆', '四')\
                                  .replace('伍', '五').replace('陆', '六')\
                                  .replace('柒', '七').replace('捌', '八')\
                                  .replace('玖', '九')\
                                  .replace('拾', '十').replace('佰', '百')\
                                  .replace('仟', '千').replace('萬', '万')\
                                  .replace('億', '亿').replace('圓', '元')\
                                  .replace('整', '')
        ...
        if '万' in cleaned:
            ...
            if len(parts) >= 1:
                ...
                # 计算万位部分的值
                wan_value = self._convert_chinese_segment(wan_part, char_to_digit, unit_map)
                
                # 计算万以下部分的值
                below_wan_value = self._convert_chinese_segment(below_wan_part, char_to_digit, unit_map) if below_wan_part else 0
        ...
        while i < len(segment):
            ...
            if char in char_to_digit:
                ...
                if i + 1 < len(segment) and segment[i + 1] in unit_map:
                    ...
                    if i == len(segment) - 1 or (i + 1 < len(segment) and segment[i + 1] not in unit_map):
                        temp += digit
                    i += 1

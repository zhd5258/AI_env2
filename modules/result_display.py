...
class ResultDisplay:
    ...
    def generate_multi_level_table(self) -> Dict[str, Any]:
        ...
        try:
            # 获取评分规则
            scoring_rules = (
                self.session.query(ScoringRule)
                    ...
                    'children': [
                        {
                            'name': '价格分',
                            'max_score': float(
                                getattr(rule, 'Parent_max_score', 0) or 0
                            ),
                            'description': getattr(rule, 'description', ''),
                        }
                    ],
            ...
            try:
                ...
                if isinstance(detailed_scores, list):
                    for score_item in detailed_scores:
                        child_item_name = score_item.get(
                            'Child_Item_Name'
                        ) or score_item.get('criteria_name')
    ...
    def _generate_headers(
        self, rules_tree: List[Dict[str, Any]]
        ...
        # 第一行表头
        header_row1 = [{'name': '排名', 'rowspan': 2}, {'name': '投标人', 'rowspan': 2}]
        ...
        for parent_item in rules_tree:
            ...
            if children:
                ...
                # 分离价格分项和其他子项
                non_price_children = [
                    child for child in children if child.get('name') != '价格分'
                ]

                if non_price_children:
                    # 如果有非价格分的子项，在第一行添加父项并设置colspan
                    header_row1.append(
                        {'name': parent_header, 'colspan': len(non_price_children)}
                    )
    ...
    def _generate_data(
        ...
        for doc in bid_documents:
            ...
            if (
                doc.analysis_result
                and doc.analysis_result.project_id == self.project_id
            ):
        ...
        for rank, (bidder_name, total_score) in enumerate(bidder_scores_with_total, 1):
            ...
            for doc in bid_documents:
                if (
                    doc.bidder_name == bidder_name
                    and doc.analysis_result
                    and doc.analysis_result.project_id == self.project_id
                ):
                    bid_doc = doc
                    break
            ...
            if (
                bid_doc
                and bid_doc.analysis_result
                and bid_doc.analysis_result.price_score is not None
            ):
        ...
        try:
            ...
            for bid_doc in bid_documents:
                ...
                if (
                    ...
                    and bid_doc.analysis_result.project_id == self.project_id
                ):
                    summary_data.append(
                        {

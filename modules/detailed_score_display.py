...
class DetailedScoreDisplay:
    ...
    def generate_detailed_score_table(self) -> Dict[str, Any]:
        ...
        try:
            ...
            data = self._generate_detailed_score_data(
                bid_documents, detailed_scores_data, rules_tree
            )
            ...
            for i, item in enumerate(price_data, 1):
                data.append(
                    [
                        i,
                        item['bidder_name'],
                        f'¥{item["extracted_price"]:,.2f}'
                        if item['extracted_price'] is not None
                        else 'N/A',
                        round(item['price_score'], 2)
                        if item['price_score'] is not None
                        else 'N/A',
                    ]
                )
    ...
    def _generate_detailed_score_headers(
        ...
        for parent_item in rules_tree:
            ...
            if children:
                ...
                header_row1.append(
                    {'name': parent_name, 'colspan': len(children) * 2}
                )  # 每个子项有评分和理由两列
    ...
    def _generate_detailed_score_data(
        self,
